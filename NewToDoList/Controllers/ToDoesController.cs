...
namespace NewToDoList.Controllers
{
    public class ToDoesController : Controller
    {
        ...
        {
            string currentUserId = User.Identity.GetUserId();       //Get current users Id.
            ApplicationUser currentUser = db.Users.FirstOrDefault   //Get First of Default User Information
        ...
        public ActionResult Create([Bind(Include = "Id,Description,IsDone")] ToDo toDo)
        {
            if (ModelState.IsValid)
            {
                string currentUserId = User.Identity.GetUserId();       //Get current users Id.
                ApplicationUser currentUser = db.Users.FirstOrDefault   //Get First of Default User Information
                    (x => x.Id == currentUserId);                       //Lamda exp that compares user Id. to user Id in the object;
                toDo.User = currentUser;                                //Sets todo item to the current user;
                db.ToDos.Add(toDo);
                db.SaveChanges();
